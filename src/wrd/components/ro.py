...
def set_ro_op_conditions(blk):
    ...
    # blk.unit = train.find_component(f"ro_stage_{s}")
    ...
    blk.unit.area.setub(1e6)  # These ubs make is equation feasible- A = L*W
...
def init_ro(blk):
    ...
    blk.disposal.properties[0].flow_vol_phase["Liq"]
...
def report_ro(blk, w=30):
    ...
    print(
        f'{f"Inlet Flow (MGD)":<{w}s}{value(pyunits.convert(blk.feed.properties[0].flow_vol_phase["Liq"], to_units=pyunits.Mgallons / pyunits.day)):<{w}.3f}{"MGD"}'
    )
    print(
        f'{f"Brine Flow (MGD)":<{w}s}{value(pyunits.convert(blk.disposal.properties[0].flow_vol_phase["Liq"], to_units=pyunits.Mgallons / pyunits.day)):<{w}.3f}{"MGD"}'
    )
    print(
        f'{f"Product Flow (MGD)":<{w}s}{value(pyunits.convert(blk.product.properties[0].flow_vol_phase["Liq"], to_units=pyunits.Mgallons / pyunits.day)):<{w}.3f}{"MGD"}'
    )
    print(
        f'{f"Recovery":<{w}s}{value(blk.unit.recovery_vol_phase[0, "Liq"])*100:<{w}.3f}{"%"}'
    )
    # print(
    #     f'{f"Inlet Flow (MGD)":<{w}s}{value(pyunits.convert(blk.unit.feed_side.properties[0, 0].flow_vol_phase["Liq"], to_units=pyunits.Mgallons / pyunits.day)):<{w}.3f}{"MGD"}'
    # )
    print(
...
if __name__ == "__main__":
    ...
    set_ro_op_conditions(m.fs.ro, file=file, stage=1)
    ...
    # m.fs.ro.unit.feed_side.cp_modulus.display()
    # m.fs.ro.unit.config.display()
    report_ro(m.fs.ro)
    # m.fs.ro.unit.feed_side.volume
    m.fs.ro.unit.report()
    # print(*m.fs.ro.unit.feed_side.properties.index_set(), sep="\n")
