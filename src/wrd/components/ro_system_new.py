...
def set_inlet_conditions(m, Qin=2637*3, Cin=0.5, file="wrd_ro_inputs_8_19_21.yaml"):
...
def set_ro_system_op_conditions(m):
    ...
    for i in m.fs.trains:
        ...
        if i != m.fs.trains.first():
            m.fs.feed_separator.split_fraction[0, f"train{i}", "H2O"].fix(m.fs.feed_separator.even_split)
            m.fs.feed_separator.split_fraction[0, f"train{i}", "NaCl"].fix(m.fs.feed_separator.even_split)
        else:
            m.fs.feed_separator.split_fraction[0, f"train{i}", "H2O"].set_value(m.fs.feed_separator.even_split)
            m.fs.feed_separator.split_fraction[0, f"train{i}", "NaCl"].set_value(m.fs.feed_separator.even_split)
...
if __name__ == "__main__":
    ...
    m = build_ro_system(num_trains=3, num_stages=3)
    set_ro_system_scaling(m)
    calculate_scaling_factors(m)
    set_inlet_conditions(m, Qin=2637, Cin=0.5)
    set_ro_system_op_conditions(m)
    print(f"Dof of system: {degrees_of_freedom(m)}")
    # m.fs.feed_separator.split_fraction.display()
